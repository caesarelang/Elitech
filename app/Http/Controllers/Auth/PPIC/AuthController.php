...
namespace App\Http\Controllers\Auth\PPIC;
...
{
    /**
     * Login untuk modul PPIC
     */
    public function loginPPIC(Request $request)
    {
        ...
        if (!$user || !Hash::check($validated['password'], $user->password)) {
            return response()->json([
                'message' => 'Email atau password salah, atau Anda tidak memiliki akses ke modul PPIC'
            ], 401);
        }

        if ($user->module !== 'ppic') {
            return response()->json([
                'message' => 'Anda tidak memiliki akses ke modul PPIC'
            ], 403);
        }

        $abilities = ['module:ppic'];
        ...
        $token = $user->createToken('ppic-auth-token', $abilities)->plainTextToken;
