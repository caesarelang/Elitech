<template>
  <div class="p-6 max-w-3xl mx-auto bg-white rounded shadow">
    <div class="flex items-center gap-6 mb-6">
      ...
      />
      <div v-else class="w-24 h-24 rounded-full bg-green-300 flex items-center justify-center text-xl font-bold text-gray-700 border">
        {{ userInitials }}
      </div>
      <div>
        <h1 class="text-2xl font-bold">{{ user.name || 'N/A' }}</h1>
        <p class="text-gray-600">{{ user.role || 'Staff PPIC' }}</p>
      </div>
    </div>
